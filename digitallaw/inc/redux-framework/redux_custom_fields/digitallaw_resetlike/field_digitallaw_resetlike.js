jQuery( document ).ready(function($) {
	...
	jQuery('#digitallaw_resetlike_btn_caller').click(function() {
	...
	// NO link
	jQuery('#digitallaw_resetlike_btn_no').click(function() {
		jQuery('#tm-resetlike-wrapper').slideUp();
		jQuery('#digitallaw_resetlike_btn_caller').removeAttr('disabled');
		return false;
	});
	...
	jQuery('#digitallaw_resetlike_btn').click(function() {
		...
		$.ajax({
			...
			error: function(request, status, error) {
				//resultDiv.html( '<p><strong style="color: red">' + digitallawVars.strError + ": " + request.status + '</p>' );
		...
		/*
		var data = {
			'action'                : 'thememount_one_click_demo',
