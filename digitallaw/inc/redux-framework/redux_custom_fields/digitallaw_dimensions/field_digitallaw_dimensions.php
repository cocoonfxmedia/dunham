<?php
    ...
    if ( ! class_exists( 'ReduxFramework_digitallaw_dimensions' ) ) {
        class ReduxFramework_digitallaw_dimensions {
            ...
            function render() {
                ...
                echo '<fieldset id="' . $this->field['id'] . '" class="redux-tm-dimensions-container" data-id="' . $this->field['id'] . '">';
                ...
                if ( $this->field['width'] === true ) {
                    ...
                    echo '<div class="field-dimensions-input input-prepend input-append">';
                    echo '<span class="add-on"><i class="el el-icon-resize-horizontal icon-large"></i></span>';
                    echo '<input type="text" name="' . $this->field['name'] . $this->field['name_suffix']. '[width]' . '" class="redux-tm-dimensions-input redux-tm-dimensions-width mini' . $this->field['class'] . '" placeholder="' . esc_html__( 'Width', 'digitallaw' ) . '" rel="' . $this->field['id'] . '-width" value="' . filter_var( $this->value['width'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION ) . '">';
                    //echo '<input data-id="' . $this->field['id'] . '" type="hidden" id="' . $this->field['id'] . '-width" name="' . $this->field['name'] . $this->field['name_suffix']. '[width]' . '" value="' . $this->value['width'] . '">';
					echo '<span class="add-on"> px</span>';
					echo '</div>';
                }
                ...
                if ( $this->field['height'] === true ) {
                    ...
                    echo '<div class="field-tm-dimensions-input input-prepend input-append">';
                    echo '<span class="add-on"><i class="el el-icon-resize-vertical icon-large"></i></span>';
                    echo '<input type="text" name="' . $this->field['name'] . $this->field['name_suffix']. '[height]' . '" class="redux-tm-dimensions-input redux-tm-dimensions-height mini' . $this->field['class'] . '" placeholder="' . esc_html__( 'Height', 'digitallaw' ) . '" rel="' . $this->field['id'] . '-height" value="' . filter_var( $this->value['height'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION ) . '">';
                    //echo '<input data-id="' . $this->field['id'] . '" type="hidden" id="' . $this->field['id'] . '-height" name="' . $this->field['name'] . $this->field['name_suffix']. '[height]' . '" value="' . $this->value['height'] . '">';
					echo '<span class="add-on"> px</span>';
					echo '</div>';
				...
				echo '
					<strong>' . esc_html__( 'Hard Crop Image?', 'digitallaw' ) . '</strong>
					 &nbsp; 
					<label class="radio inline">
					
						<input type="radio" class="radio ' . $this->field['class'] . '" id="' . $this->field['id'] . '_yes" name="' . $this->field['name'] . $this->field['name_suffix'] . '[crop]" value="yes" ' . checked( $this->value['crop'], 'yes', false ) . '/>
					
						'.esc_html__('YES','digitallaw').'
					</label>
					 &nbsp; 
					<label class="radio inline">
					
						<input type="radio" class="radio ' . $this->field['class'] . '" id="' . $this->field['id'] . '_no" name="' . $this->field['name'] . $this->field['name_suffix'] . '[crop]" value="no" ' . checked( $this->value['crop'], 'no', false ) . '/>
            ...
            function enqueue() {
                ...
                //if ($this->parent->args['dev_mode']) {
                    wp_enqueue_style(
                        'redux-field-tm-dimensions-css',
                        //ReduxFramework::$_url . 'inc/fields/dimensions/field_dimensions.css',
						get_template_directory_uri() . '/inc/redux-framework/redux_custom_fields/digitallaw_dimensions/field_digitallaw_dimensions.css',
                        array(),
                        time(),
                        'all'
                    );
            ...
            public function output() {
				...
				$crop   = isset( $this->field['mode'] ) && ! empty( $this->field['mode'] ) ? $this->field['mode'] : 'crop';
                ...
                $cleanValue = array(
					...
					$crop  => isset( $this->value['crop'] ) ? $this->value['crop'] : 'yes',
                );
